...
export function JournalView({ entries, setEntries, items, setItems, refreshEntries, refreshItems, loading, setError, hasMore, loadMore }) {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      setContent(''); setLocation(null); void waitForAi(created.id);
  ...
  const waitForAi = async (entryId) => {
    for (let attempt = 0; attempt < 15; attempt += 1) {
      ...
      try {
        ...
        if (!entry || entry.processingStatus !== 'PENDING') { await refreshItems(); return; }
      } catch (requestError) { if (attempt === 14) setError(requestError.message); }
    }
  };
