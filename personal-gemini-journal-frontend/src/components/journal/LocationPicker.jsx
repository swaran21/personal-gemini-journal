...
export function LocationPicker({ location, onChange, setError }) {
  ...
  const locate = () => {
    if (cooldown) { setError('Please wait a minute before requesting location again. You can always journal without it.'); return; }
    if (!navigator.geolocation) { setError('Location is not supported by this browser.'); return; }
    setCooldown(true); window.setTimeout(() => setCooldown(false), 60_000);
    setLocating(true);
  ...
  if (!location) return <button type="button" onClick={locate} disabled={locating || cooldown} className="mt-3 inline-flex items-center gap-2 rounded-xl border border-[#DCDCD2] bg-white px-3 py-2 text-xs font-semibold text-[#7A8D80] hover:bg-[#F9F8F4] disabled:opacity-60">{locating ? <Loader2 className="h-4 w-4 animate-spin" /> : <LocateFixed className="h-4 w-4" />}{cooldown ? 'Location ready shortly' : 'Add current location'}</button>;
