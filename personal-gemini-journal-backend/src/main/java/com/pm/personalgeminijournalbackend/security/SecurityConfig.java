...
public class SecurityConfig {
    ...
    @Bean CorsConfigurationSource corsConfigurationSource(ApplicationConfig.CorsProperties properties) {
        ...
        if (properties.getAllowedOrigins() != null && !properties.getAllowedOrigins().isBlank()) {
            config.setAllowedOrigins(Arrays.stream(properties.getAllowedOrigins().split(","))
                    .map(String::trim).filter(origin -> !origin.isBlank()).toList());
        }
        config.setAllowedMethods(List.of("POST", "PATCH", "DELETE", "GET", "OPTIONS")); config.setAllowedHeaders(List.of("Authorization", "Content-Type")); config.setMaxAge(3600L);
