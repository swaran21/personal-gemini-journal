...
For local properties, copy `src/main/resources/application-local.properties.example` to `src/main/resources/application-local.properties`, fill in the Secret Manager resource name, and start with `--spring.profiles.active=local`. The real file is ignored by Git and the application never accepts a raw Gemini API key. The template includes non-secret Google Cloud project and Firestore database identifiers; Firebase web configuration belongs only in the separate frontend project.
